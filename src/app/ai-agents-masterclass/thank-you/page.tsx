...
// ═══════════════════════════════════════════════════════════════════════════
// EDIT THIS: paste your AI Agents Masterclass WhatsApp COMMUNITY invite link.
// (WhatsApp → group → Invite via link → copy the chat.whatsapp.com URL.)
// ═══════════════════════════════════════════════════════════════════════════
const WHATSAPP_GROUP_LINK = "https://chat.whatsapp.com/REPLACE_WITH_YOUR_GROUP_INVITE";

const CLASS = { date: "Sunday, 23 August 2026", time: "11:00 AM IST" };
...
function Confetti() {
  ...
  useEffect(() => {
    const colors = ["#4f46e5", "#6366f1", "#818cf8", "#a5b4fc", "#22c55e", "#3b82f6", "#8b5cf6"];
    ...
    <div aria-hidden="true" style={{ position: "fixed", inset: 0, pointerEvents: "none", overflow: "hidden", zIndex: 60 }}>
      {pieces.map((p) => (
        <span key={p.id} style={{
          ...
          animation: `agty-confetti ${p.dur}s cubic-bezier(0.2,0.6,0.4,1) ${p.delay}s forwards`, ["--drift" as string]: `${p.drift}px`,
...
export default function AiAgentsThankYouPage() {
  useEffect(() => {
    if (typeof window !== "undefined" && typeof window.fbq === "function") {
      window.fbq("track", "CompleteRegistration", { content_name: "AI Agents Masterclass" });
    }
  }, []);

  return (
    <div id="agty-top" style={{ background: "#f8fafc", minHeight: "100vh", color: "#0f172a" }}>
      <Confetti />
      <style>{`
        ...
        @keyframes agty-pop{0%{transform:scale(0.6);opacity:0}60%{transform:scale(1.08)}100%{transform:scale(1);opacity:1}}
        @keyframes agty-confetti{0%{transform:translate(0,0) rotate(0deg);opacity:1}100%{transform:translate(var(--drift),105vh) rotate(720deg);opacity:0.9}}
        #agty-top{font-family:'Plus Jakarta Sans',-apple-system,sans-serif;-webkit-font-smoothing:antialiased}
        #agty-top h1,#agty-top h2{font-family:'Poppins','Plus Jakarta Sans',sans-serif;letter-spacing:-0.02em}
        #agty-top .wa-btn{background:linear-gradient(135deg,#1fa855,#25D366);box-shadow:0 14px 34px rgba(37,211,102,0.45)}
        #agty-top .wa-btn:hover{background:linear-gradient(135deg,#189048,#1fbf5c);box-shadow:0 18px 42px rgba(37,211,102,0.55)}
        #agty-top .grad{background:linear-gradient(135deg,#4f46e5,#6366f1);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;color:transparent}
      `}</style>
      ...
      <div style={{ background: "linear-gradient(90deg,#4338ca,#6366f1,#4338ca)", padding: "11px 16px" }}>
        <p className="text-center font-black text-white" style={{ fontSize: 15 }}>
          ✅ You&apos;re registered for the AI Agents Masterclass!
        </p>
      </div>

      <div className="max-w-2xl mx-auto px-6 py-12 lg:py-16 text-center">
        <div className="inline-flex mb-5" style={{ animation: "agty-pop 0.5s ease both" }}>
        ...
        <p style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.14em", color: "#4f46e5", marginBottom: 10 }}>Seat reserved</p>
        ...
        <p style={{ fontSize: 17, color: "#475569", lineHeight: 1.65, fontWeight: 500, maxWidth: 520, margin: "0 auto 8px" }}>
          Your free seat is saved for <b>{CLASS.date}</b> at <b>{CLASS.time}</b>.
        </p>
        <p style={{ fontSize: 15.5, color: "#64748b", maxWidth: 520, margin: "0 auto 30px" }}>
          One important step left 👇
        </p>
        ...
        <p className="mt-8" style={{ fontSize: 15, fontWeight: 700, color: "#0f172a" }}>Come ready to build your first AI agent. ⚡</p>
        <p className="mt-2" style={{ fontSize: 13, color: "#94a3b8" }}>
          Question?{" "}
          <a href="https://wa.me/918956146485?text=Hi%2C+I+just+registered+for+the+AI+Agents+Masterclass" className="underline" style={{ color: "#4f46e5" }}>Message us on WhatsApp</a>
        </p>
      </div>
