      ...
      <div className="max-w-3xl mx-auto px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          <div className="space-y-5 text-[#52525b] text-lg leading-relaxed">
            ...
            <p>They fail because they <span className="text-foreground">forget, overthink</span>, or try to change everything at once.</p>
            <p className="text-foreground font-medium">Daily Upgrade Club is built for real life.</p>
          </div>
          ...
          <div className="mt-10 grid grid-cols-1 gap-0">
            {[
              ...
              <div key={line} className={`py-4 border-b border-border-subtle flex items-center gap-4 ${i === 4 ? "border-b-0" : ""}`}>
                <span className="text-[#25d366] font-bold font-display text-sm tabular-nums" style={{ letterSpacing: "-0.02em" }}>0{i + 1}</span>
                <p className="text-foreground font-medium text-base">{line}</p>
              </div>
            ))}
          </div>

          <div className="mt-10 pt-8 border-t border-border-subtle">
            <p className="text-[#71717a] text-base italic mb-6">No pressure. No perfection. Just daily momentum.</p>
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-full overflow-hidden border border-border">
                <img src="/hpc-logo.png" alt="Rohan" className="w-full h-full object-cover" />
              </div>
              <div>
                <p className="text-foreground font-bold text-sm">Rohan</p>
                <p className="text-[#71717a] text-xs">Founder, High Performance Club</p>
              </div>
            </div>
          </div>
        </div>
