        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          <div className="lg:col-span-7">
            ...
            <h1 className={`font-display transition-all duration-700 delay-100 ${show ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
              style={{ fontSize: "clamp(2.8rem, 7vw, 5.5rem)", lineHeight: 1.02, letterSpacing: "-0.03em", fontWeight: 800, color: "#18181b" }}>
              One habit.<br />
            ...
            {/* Divider */}
            <div className={`my-8 h-px bg-[#25d366] transition-all duration-700 delay-200 ${show ? "opacity-100 w-16" : "opacity-0 w-0"}`} style={{ width: show ? 64 : 0 }} />

            {/* Subhead */}
            <p className={`text-foreground-muted text-lg sm:text-xl leading-relaxed max-w-lg transition-all duration-700 delay-300 ${show ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
            ...
            <p className={`text-foreground-subtle text-base mt-3 transition-all duration-700 delay-[350ms] ${show ? "opacity-100" : "opacity-0"}`}>
            ...
            {/* Stats row */}
            <div className={`mt-12 grid grid-cols-4 gap-0 bg-white/70 border border-border-subtle rounded-2xl overflow-hidden transition-all duration-700 delay-700 ${show ? "opacity-100" : "opacity-0"}`}>
              {STATS.map((s, i) => (
          ...
          <div className={`lg:col-span-5 flex justify-center transition-all duration-700 delay-300 ${show ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            <div className="relative">
              <div className="absolute inset-0 blur-3xl opacity-20 scale-75" style={{ background: "radial-gradient(circle, #25d366 0%, transparent 70%)" }} />
              <div className="relative">
                <DUCPhone />
              </div>
              {/* Floating label */}
              <div className="absolute -left-4 lg:-left-20 top-1/4 hidden lg:block">
              ...
              {/* Sticky note style */}
              <div className="absolute -right-2 lg:-right-10 bottom-1/4 hidden lg:block sticky-note px-4 py-3 rounded-lg tilt-right">
