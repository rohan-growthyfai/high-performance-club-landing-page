      ...
      <div className="max-w-5xl mx-auto px-6 lg:px-8">
        ...
        <div className="mb-16">
          ...
          <h2 style={{ fontSize: "clamp(2rem, 4vw, 3rem)", lineHeight: 1.1, letterSpacing: "-0.025em", fontWeight: 800, color: "#18181b" }}>
            Four steps.<br />
            <span style={{ color: "#71717a", fontStyle: "italic", fontWeight: 400 }}>Repeat for 30 days.</span>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {steps.map((step, i) => (
            <div key={step.num} className="bg-white border border-border-subtle rounded-2xl p-7 relative group hover:shadow-md transition-shadow">
              {/* Step number — huge */}
              <p className="font-display font-black mb-6 select-none" style={{ fontSize: "4rem", lineHeight: 1, color: step.color, opacity: 0.25, letterSpacing: "-0.04em" }}>{step.num}</p>
              <div className="w-8 h-0.5 mb-5 rounded-full" style={{ background: step.color }} />
              <p className="text-foreground font-bold text-base mb-2 leading-snug">{step.title}</p>
              <p className="text-foreground-muted text-sm leading-relaxed">{step.desc}</p>
              {/* Arrow */}
              {i < steps.length - 1 && (
                <div className="hidden lg:block absolute top-1/2 -right-3 -translate-y-1/2 z-10 text-foreground-subtle text-xl">›</div>
              )}
            </div>
          ))}
        </div>
